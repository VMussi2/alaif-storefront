...
async function renderShop() {
  let data;
  try {
    data = await (await fetch("products.json", { cache: "no-store" })).json();
  } catch (err) {
...
$$("[data-illo]").forEach(el => {
  ...
  fetch(`assets/${el.dataset.illo}.svg`).then(r => r.ok ? r.text() : null)
    .then(svg => { if (svg) el.innerHTML = svg; }).catch(() => {});
});
$$("[data-doodle]").forEach(el => {
  if (el.children.length) return;
  fetch(`assets/doodles/${el.dataset.doodle}.svg`).then(r => r.ok ? r.text() : null)
    .then(svg => { if (svg) el.innerHTML = svg; }).catch(() => {});
});
